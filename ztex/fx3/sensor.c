        ...
        uint8_t lowData)
{
   ...
   // preamble.buffer[2] = lowAddr;
    ...
    //buf[0] = highData;
    buf[0] = lowData;
CyU3PDebugPrint (4, "Inside write I2c : slaveaddr = 0x%x, reg = 0x%x, buf[0] = 0x%x\r\n",preamble.buffer[0], preamble.buffer[1],buf[0]); 
    apiRetStatus = CyU3PI2cTransmitBytes (&preamble, buf, 1, 0);
	CyU3PDebugPrint (4, "write I2c returned : %d\r\n",apiRetStatus);
        ...
        uint8_t *buf)
{
    ...
    CyU3PDebugPrint (4, "Inside write I2c multibyte: slaveaddr = 0x%x, reg = 0x%x, buf[0] = 0x%x,buf[1] = 0x%x,buf[2] = 0x%x,buf[3] = 0x%x\n", preamble.buffer[0], preamble.buffer[1],buf[0],buf[1],buf[2],buf[3]); 
    apiRetStatus = CyU3PI2cTransmitBytes (&preamble, buf, count, 0);
	CyU3PDebugPrint (4, " Return from I2C write multibyte : %d\r\n",apiRetStatus);
    SensorI2CAccessDelay (apiRetStatus);
        ...
        uint8_t *buf)
{
    ...
    SensorI2CAccessDelay (apiRetStatus);
CyU3PDebugPrint (4, "I2C Read : Slave address = 0x%x, reg addr = 0x%x, return val = %d buf[0] = 0x%x\r\n",slaveAddr,preamble.buffer[1],apiRetStatus, buf[0]);
    return apiRetStatus;
}
...
/*
   Get the current led brightness setting from the MT9M114 sensor.
 */
uint8_t
SensorGetBrightness (
        void)
{
    uint8_t buf[4];
	uint32_t tmp = 0;
    CyU3PDebugPrint(4,"---------LED--- SensorGetBrightness----------\r\n");


  SensorRead(SENSOR_ADDR_RD,0x0c,0x00,0x04,buf);
  CyU3PDebugPrint (4, " Mode config return : buf[0] = 0x%x, buf[1] = 0X%x,buf[2] = 0x%x,buf[3] = 0X%x\r\n",buf[0],buf[1],buf[2],buf[3]);
  tmp = (uint32_t)buf[0]<<24 | (uint32_t)buf[1] <<16 | (uint32_t)buf[2] <<8 |buf[3];
  CyU3PDebugPrint (4, "tmp = %x\r\n",tmp);
  return tmp;
...
/*
   Get the current ad h1 setting from the sensor.
 */
uint8_t
SensorGetAD (
        void)
{
    uint8_t buf[4];
	uint32_t tmp = 0;
    CyU3PDebugPrint(4,"------------ SensorGetBrightness--H1--------\r\n");
...
/*
   Get the current ad h2 setting from the sensor.
 */
uint8_t
SensorGetAD1 (
        void)
{
    uint8_t buf[4];
	uint32_t tmp = 0;
    CyU3PDebugPrint(4,"------------ SensorGetBrightness--H2--------\r\n");
...
/*
   Get the current ad rg setting from the sensor.
 */
uint8_t
SensorGetAD2(
        void)
{
    uint8_t buf[4];
	uint32_t tmp = 0;
    CyU3PDebugPrint(4,"------------ SensorGetBrightness--RG--------\r\n");
        ...
        void)
{
    uint8_t buf[4];
	uint32_t tmp = 0;
    CyU3PDebugPrint(4,"------------ TO on white or blue LED----------\r\n");
...
/*
   Update the LED brightness setting for the sensor.
 */
void
SensorSetBrightness (
        uint8_t * input, int size)
{
	CyU3PDebugPrint (4, " Inside fn. SensorSetBrightness : setting input : %x  \r\n",*input);
...
/*
   Update the CALIBRATION setting for the AD9979...........H1
 */
void
SensorSetAD(
        uint8_t * ad_input, int ad_size)
{
	CyU3PDebugPrint (4, " Inside fn. SensorSetBrightness : setting input : %x  \r\n",*ad_input);
...
uint8_t ad_buf[4];
	...
	ad_buf[0]=0x00;
	ad_buf[1]=0x01;
	ad_buf[2]=0x20 + *ad_input ;
	ad_buf[3]=0x00 + *ad_input;
	SensorWrite(SENSOR_ADDR_WR,0x00,0x00,4,ad_buf); //H1

}


/*
   Update the CALIBRATION setting for the AD9979...........H2
 */
void
SensorSetAD1(
        uint8_t * ad1_input, int ad1_size)
{
	CyU3PDebugPrint (4, " Inside fn. SensorSetBrightness : setting input : %x  \r\n",*ad1_input);
...
uint8_t ad1_buf[4];
	...
	ad1_buf[0]=0x00;
	ad1_buf[1]=0x01;
	ad1_buf[2]=0x00 + *ad1_input ;
	ad1_buf[3]=0x20 + *ad1_input;
	SensorWrite(SENSOR_ADDR_WR,0x04,0x00,4,ad1_buf); //H2

}

/*
   Update the CALIBRATION setting for the AD9979...........RG
 */
void
SensorSetAD2(
        uint8_t * ad2_input, int ad2_size)
{
	CyU3PDebugPrint (4, " Inside fn. SensorSetBrightness : setting input : %x  \r\n",*ad2_input);
...
uint8_t ad2_buf[4];
	...
	ad2_buf[0]=0x00;
	ad2_buf[1]=0x01;
	ad2_buf[2]=0x00 + *ad2_input ;
	ad2_buf[3]=0x0c + *ad2_input;
	SensorWrite(SENSOR_ADDR_WR,0x08,0x00,4,ad2_buf); //RG

}


/*
   To select white or blue led
 */
void
SensorSetLED(
        uint8_t * LED_input, int LED_size)
{
	CyU3PDebugPrint (4, " Inside fn. SensorSetBrightness : setting input : %x  \r\n",*LED_input);
