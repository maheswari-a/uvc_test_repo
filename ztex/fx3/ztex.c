...
static void UVCHandleProcessingUnitRqts(void) {
	...
	case CY_FX_UVC_PU_BRIGHTNESS_CONTROL:
		...
		case CY_FX_USB_UVC_GET_LEN_REQ: /* Length of brightness data = 1 byte. */
			glEp0Buffer[0] = 1;
			CyU3PDebugPrint(4, "Inside case CY_FX_USB_UVC_GET_LEN_REQ\r\n");
		...
		case CY_FX_USB_UVC_GET_CUR_REQ: /* Current brightness value. */
			...
			// CyU3PUsbSendEP0Data (1, (uint8_t *)glEp0Buffer);
			CyU3PDebugPrint(4, "Inside case CY_FX_USB_UVC_GET_CUR_REQ\r\n");
			uint32_t ptr = 0;
			ptr = SensorGetBrightness();
			//CyU3PDebugPrint (4, "value returned from sensor read ptr[0]= %x,ptr[1]= %x,ptr[2]= %x,ptr[3]= %x\r\n",ptr[0],ptr[1],ptr[2],ptr[3]);
			CyU3PDebugPrint(4, "value returned from sensor read ptr = %x\r\n",
					ptr);
			CyU3PUsbSendEP0Data(1, (uint8_t *) ptr);
			break;
		...
		case CY_FX_USB_UVC_SET_CUR_REQ: /* Update brightness value. */
			...
			apiRetStatus = CyU3PUsbGetEP0Data(wLength, //CY_FX_UVC_MAX_PROBE_SETTING_ALIGNED,

					glEp0Buffer, &readCount);

			CyU3PDebugPrint(4,
					"Inside case SET_CUR_REQ : apiRetStatus = %d glEp0Buffer[0] = %x,glEp0Buffer[1] = %x, glEp0Buffer[2] = %x,glEp0Buffer[3] = %x,readCount = %d\r\n",
					apiRetStatus, glEp0Buffer[0], glEp0Buffer[1],
					glEp0Buffer[2], glEp0Buffer[3], readCount);

			if (apiRetStatus == CY_U3P_SUCCESS) {
				CyU3PDebugPrint(4, "Inside bfr calling set brightness\r\n");
				//SensorSetBrightness (glEp0Buffer[0]);
				SensorSetBrightness((uint8_t *) &glEp0Buffer, (int) readCount);
			}
			break;
		default:
			CyU3PUsbStall(0, CyTrue, CyFalse);
...
/*... ad9979 CALIBRATION... H1....#define CY_FX_UVC_PU_GAIN_CONTROL (uint16_t)(0x0400)*/
...
case CY_FX_UVC_PU_GAIN_CONTROL:
	switch (bRequest)
	{
		...
		case CY_FX_USB_UVC_GET_CUR_REQ: /* Current brightness value. */
			...
			ptr1 = SensorGetAD ();
			CyU3PDebugPrint (4, "value returned from sensor read ptr = %x\r\n",ptr1);
			CyU3PUsbSendEP0Data (1, (uint8_t *)ptr1);
			break;
		case CY_FX_USB_UVC_GET_MIN_REQ: /* Minimum brightness = 0. */
			glEp0Buffer[0] = 0;
			CyU3PUsbSendEP0Data (1, (uint8_t *)glEp0Buffer);
		...
		case CY_FX_USB_UVC_SET_CUR_REQ: /* Update brightness value. */
			apiRetStatus = CyU3PUsbGetEP0Data (wLength,//CY_FX_UVC_MAX_PROBE_SETTING_ALIGNED,
					glEp0Buffer, &readCount);
			CyU3PDebugPrint (4, "Inside case SET_CUR_REQ : apiRetStatus = %d glEp0Buffer[0] = %x,glEp0Buffer[1] = %x, glEp0Buffer[2] = %x,glEp0Buffer[3] = %x,readCount = %d\r\n",apiRetStatus,glEp0Buffer[0],glEp0Buffer[1],glEp0Buffer[2],glEp0Buffer[3],readCount);


			if (apiRetStatus == CY_U3P_SUCCESS)
			{
				CyU3PDebugPrint (4, "Inside bfr calling set brightness\r\n");
				SensorSetAD ((uint8_t *)&glEp0Buffer,(int)readCount);
			}
			break;
		default:
			CyU3PUsbStall (0, CyTrue, CyFalse);
...
/*... ad9979 CALIBRATION. H2.#define CY_FX_UVC_PU_POWER_LINE_FREQUENCY_CONTROL(uint16_t)(0x0500)*/
		...
		ptr2 = SensorGetAD1 ();
		CyU3PDebugPrint (4, "value returned from sensor read ptr = %x\r\n",ptr2);
		CyU3PUsbSendEP0Data (1, (uint8_t *)ptr2);
		break;
		case CY_FX_USB_UVC_GET_MIN_REQ: /* Minimum brightness = 0. */
		glEp0Buffer[0] = 0;
		CyU3PUsbSendEP0Data (1, (uint8_t *)glEp0Buffer);
		...
		{
		CyU3PDebugPrint (4, "Inside bfr calling set brightness\r\n");
		SensorSetAD1 ((uint8_t *)&glEp0Buffer,(int)readCount);
		}
		break;
		default:
		CyU3PUsbStall (0, CyTrue, CyFalse);
		...
		break;
/*... ad9979 CALIBRATION..RG...#define CY_FX_UVC_PU_HUE_CONTROL  (uint16_t)(0x0600)*/
...
case CY_FX_UVC_PU_HUE_CONTROL:
	switch (bRequest)
	{
		...
		case CY_FX_USB_UVC_GET_CUR_REQ: /* Current brightness value. */
			...
			ptr3 = SensorGetAD2();
			CyU3PDebugPrint (4, "value returned from sensor read ptr = %x\r\n",ptr3);
			CyU3PUsbSendEP0Data (1, (uint8_t *)ptr3);
			break;
		case CY_FX_USB_UVC_GET_MIN_REQ: /* Minimum brightness = 0. */
			glEp0Buffer[0] = 0;
			CyU3PUsbSendEP0Data (1, (uint8_t *)glEp0Buffer);
		...
		case CY_FX_USB_UVC_SET_CUR_REQ: /* Update brightness value. */
			...
			{
				CyU3PDebugPrint (4, "Inside bfr calling set brightness\r\n");
				SensorSetAD2 ((uint8_t *)&glEp0Buffer,(int)readCount);
			}
			break;
		default:
			CyU3PUsbStall (0, CyTrue, CyFalse);
...
/*...........LED on /off  CY_FX_UVC_PU_SATURATION_CONTROL    (uint16_t)(0x0700).......................................*/
			  ...
			  switch (bRequest)
			  {
				  ...
				  case CY_FX_USB_UVC_GET_CUR_REQ: /* Current brightness value. */
					...
					uint32_t ptr4 = 0;
					  ptr4= SensorGetLED ();
					  CyU3PDebugPrint (4, "value returned from sensor read ptr = %x\r\n",ptr4);
					  CyU3PUsbSendEP0Data (1, (uint8_t *)ptr4);
					  break;
				  case CY_FX_USB_UVC_GET_MIN_REQ: /* Minimum brightness = 0. */
					  glEp0Buffer[0] = 0;
					  CyU3PUsbSendEP0Data (1, (uint8_t *)glEp0Buffer);
				  ...
				  case CY_FX_USB_UVC_SET_CUR_REQ: /* Update brightness value. */
					...
					CyU3PDebugPrint (4, "Inside case SET_CUR_REQ : apiRetStatus = %d glEp0Buffer[0] = %x,glEp0Buffer[1] = %x, glEp0Buffer[2] = %x,glEp0Buffer[3] = %x,readCount = %d\r\n",apiRetStatus,glEp0Buffer[0],glEp0Buffer[1],glEp0Buffer[2],glEp0Buffer[3],readCount);
					  ...
					  {
						CyU3PDebugPrint (4, "Inside bfr calling set brightness\r\n");
						SensorSetLED ((uint8_t *)&glEp0Buffer,(int)readCount);
					  }
					  break;
				  default:
					  CyU3PUsbStall (0, CyTrue, CyFalse);
